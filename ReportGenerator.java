 ...
 * ReportGenerator
 *
 * 탐지된 취약점을 다양한 형식(Markdown, JSON, HTML)으로 출력하는 리포트 생성기입니다.
 *
 * 리포트 항목 (가이드 요구사항 기반):
 *   [1] 진단항목 번호 (Rule ID)
 *   [2] 약점명
 *   [3] 위험도
 *   [4] 취약한 코드 라인
 *   [5] Taint 흐름 (Source → Propagator → Sink)
 *   [6] 권고 수정 코드
 *   [7] 가이드 참조 페이지
 */
public class ReportGenerator {
    ...
    public String generateMarkdown(List<Finding> findings, String targetProject) {
        ...
        sb.append("| **위험도별** | ");
        Arrays.stream(Finding.Severity.values()).forEach(s ->
        ...
        for (int i = 0; i < findings.size(); i++) {
            ...
            sb.append(String.format("| %d | `%s` | %s | %s | `%s` | %d |\n",
                    ...
                    f.getRuleId(),
                    f.getRuleName(),
                    severityBadge(f.getSeverity()),
                    shortPath(f.getFilePath()),
                    f.getLineNumber()));
        }
        ...
        for (int i = 0; i < findings.size(); i++) {
            ...
            sb.append("#### ✅ 권고 수정 코드\n\n");
            sb.append("```java\n");
            sb.append(remedy.getRemediatedCode());
            sb.append("\n```\n\n");
    ...
    public String generateJson(List<Finding> findings, String targetProject) {
        ...
        for (int i = 0; i < findings.size(); i++) {
            ...
            sb.append("      \"cweIds\": [").append(
                    f.getCweIds().stream().map(c -> "\"" + c + "\"").collect(Collectors.joining(", "))
            ).append("],\n");
            ...
            if (f.getTaintFlows() != null && !f.getTaintFlows().isEmpty()) {
                ...
                f.getTaintFlows().forEach(flow -> {
                    ...
                    sb.append(flow.getPropagators().stream()
